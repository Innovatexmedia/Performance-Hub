...
export const leadService = {
   ...
   *
   * bookings/calls/payments counts are real. deals remains stubbed until
   * that module is integrated.
   */
  async getLeadDetails(ctx, id) {
    const lead = await this.getLead(ctx, id);

    // BUG FIX: the destructured variable names here did not match the
    // actual Promise.all array order below -- position 7 (countCallsByLead)
    // was being read into `paymentCount`, and position 8
    // (countPaymentsByLead) was being read into `callCount`. That swapped
    // the real Calls and Payments counts shown in the lead drawer for
    // every lead. Reordering the destructuring to match the array fixes
    // it; the array itself was already correct.
    const [notes, timeline, noteCount, activityCount, bookingCount, qualificationCount, callCount, paymentCount] =
      await Promise.all([
