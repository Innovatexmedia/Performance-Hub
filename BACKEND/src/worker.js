...
const start = async () => {
  try {
    ...
    const redisUrlForDisplay = (() => {
      try {
        const u = new URL(config.REDIS_URL);
        return `${u.hostname}:${u.port || "6379"}`; // credential-free
      } catch {
        return "configured";
      }
    })();
    console.log(`   Redis                : ${redisUrlForDisplay}`);
    console.log(
      `   Campaign concurrency : ${config.CAMPAIGN_SEND_CONCURRENCY}`,
    );
    console.log(
      `   Campaign rate limit  : ${config.CAMPAIGN_SEND_RATE_MAX} / ${config.CAMPAIGN_SEND_RATE_DURATION_MS}ms`,
    );
    console.log(
      `   Lead import concurrency : ${config.LEAD_IMPORT_CONCURRENCY}\n`,
    );
