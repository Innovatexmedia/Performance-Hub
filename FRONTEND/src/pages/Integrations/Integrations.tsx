            ...
            <div className="space-y-4 text-sm">
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3">
                ...
                {config.config.adAccountId ? (
                  <p className="mt-1 text-sm font-medium text-ink-900">{typeof config.config.accountName === 'string' && config.config.accountName ? config.config.accountName : String(config.config.adAccountId)}</p>
                ) : (
              ...
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                <p className="text-xs font-semibold text-amber-800">Step 2 of 2 — Tracking setup (repeat for each new ad)</p>
                <p className="mt-1 text-xs text-ink-500">Step 1 alone only reports how much you spent. This step is what makes a real ad click create a correctly-tagged Lead. <strong>Unlike Google Ads, Meta has no account-wide setting for this</strong> — paste these two values into every NEW ad you create (under its "Tracking" section). The easiest way: set them up once on one ad, then always create future ads by duplicating that ad in Meta Ads Manager — duplicating carries this setup forward automatically, so you're not retyping it each time. Without this, leads from Meta Ads won't show their real source.</p>
                {trackingSetup ? (
                  <div className="mt-2 space-y-2">
                    <div>
